...
const eventValue = (event: any, keys: string[], fallback: any = ''): any => { const automatic = String(first(event, ['eventType', 'type'], first(event?.event || event?.activity || event?.locationEvent, ['eventType', 'type'], ''))).toUpperCase().includes('LOCATION') || String(first(event, ['source'], first(event?.event || event?.activity || event?.locationEvent, ['source'], ''))).toUpperCase() !== 'MANUAL'; if (automatic && keys.some(key => ['description', 'details'].includes(key))) { const title = first(event, ['title', 'name'], first(event?.event || event?.activity || event?.locationEvent, ['title', 'name'], '')); if (title) return title; const action = first(event, ['action'], first(event?.event || event?.activity || event?.locationEvent, ['action'], '')); const place = first(event, ['placeName', 'place'], first(event?.event || event?.activity || event?.locationEvent, ['placeName', 'place'], '')); return action && place ? `${action} ${place}` : action || '\u00a0'; } return first(event, keys, first(event?.event || event?.activity || event?.locationEvent, keys, fallback)); };
const eventTime = (value: any) => { const parsed = Date.parse(String(value)); return Number.isNaN(parsed) ? String(value) : new Intl.DateTimeFormat('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(new Date(parsed)); };
...
function LegacyTimeline({ events, busy, onDelete }: { events: any[]; busy: boolean; onDelete: (id: string | number) => void }) {
  ...
  const sortedEvents = [...events].sort((left, right) => Date.parse(String(eventValue(right, ['time', 'startTime', 'timestamp', 'occurredAt'], ''))) - Date.parse(String(eventValue(left, ['time', 'startTime', 'timestamp', 'occurredAt'], ''))));
...
function Timeline({ events, busy, onDelete }: { events: any[]; busy: boolean; onDelete: (id: string | number) => void }) {
  if (busy) return <div className="space-y-5 pl-1">{[1, 2, 3].map(i => <div key={i} className="flex gap-5 animate-soft-pulse"><div className="w-12 pt-2"><div className="h-3 w-10 rounded bg-[hsl(var(--muted))]" /></div><div className="h-24 flex-1 rounded-xl bg-[hsl(var(--muted))]" /></div>)}</div>;
  if (!events.length) return <LegacyTimeline events={events} busy={busy} onDelete={onDelete} />;
  const sortedEvents = [...events].sort((left, right) => Date.parse(String(eventValue(right, ['timestamp', 'time', 'startTime', 'occurredAt'], ''))) - Date.parse(String(eventValue(left, ['timestamp', 'time', 'startTime', 'occurredAt'], ''))));
  return <div className="relative space-y-0 before:absolute before:bottom-6 before:left-[58px] before:top-6 before:w-px before:bg-[hsl(var(--border))] sm:before:left-[74px]">{sortedEvents.map((event, index) => { const id = first(event, ['id', 'eventId'], index); const type = String(eventValue(event, ['eventType', 'type'], 'ACTIVITY')).toUpperCase(); const automatic = type.includes('LOCATION') || String(eventValue(event, ['source'], '')).toUpperCase() !== 'MANUAL'; const time = eventValue(event, ['timestamp', 'time', 'startTime', 'occurredAt'], '—'); const title = eventValue(event, ['title', 'name'], automatic ? `${String(eventValue(event, ['action'], 'Arrived')).toLowerCase()} ${eventValue(event, ['placeName', 'place'], 'somewhere')}` : 'Untitled moment'); const place = typeof eventValue(event, ['place'], '') === 'object' ? first(eventValue(event, ['place'], {}), ['name'], '') : eventValue(event, ['place', 'placeName'], ''); return <article key={`${id}-${index}`} className="group relative flex gap-4 py-4 sm:gap-5 animate-rise" style={{ animationDelay: `${Math.min(index * 45, 300)}ms` }} data-testid={`card-event-${id}`}><time dateTime={String(time)} className="w-11 shrink-0 pt-1 text-right font-mono text-[10px] text-[hsl(var(--muted-foreground))] sm:w-[62px]">{eventTime(time)}</time><span className={`relative z-10 mt-1.5 grid h-3 w-3 shrink-0 place-items-center rounded-full border-2 ${automatic ? 'border-[hsl(var(--accent))] bg-[hsl(var(--background))]' : 'border-[hsl(var(--primary))] bg-[hsl(var(--primary))]'}`} /><div className={`min-w-0 flex-1 rounded-xl border p-4 transition group-hover:-translate-y-0.5 ${automatic ? 'border-[hsl(var(--accent)/.3)] bg-[hsl(var(--accent)/.045)]' : 'border-[hsl(var(--border))] bg-[hsl(var(--card))]'}`}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{title}</h3>{automatic && <span className="rounded-full bg-[hsl(var(--accent)/.12)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide text-[hsl(var(--accent))]">automatic</span>}</div><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{eventValue(event, ['description', 'details'], '') || (place ? `At ${place}` : 'Recorded moment')}</p></div><button onClick={() => onDelete(id)} aria-label={`Delete ${title}`} className="rounded-md p-1.5 text-[hsl(var(--muted-foreground))] opacity-0 transition group-hover:opacity-100 hover:bg-[hsl(var(--destructive)/.1)] hover:text-[hsl(var(--destructive))]" data-testid={`button-delete-event-${id}`}><Trash2 size={15} /></button></div><div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] font-mono uppercase tracking-wide text-[hsl(var(--muted-foreground))]"><span>{eventValue(event, ['category'], '') || (automatic ? 'location' : 'personal')}</span>{place && <><span>·</span><span>{place}</span></>}</div></div></article>; })}</div>;
}
...
function TodayPage({ timelineOnly = false }: { timelineOnly?: boolean }) {
  ...
  return <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-12 lg:px-12"><header className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.22em] text-[hsl(var(--accent))]">{timelineOnly ? 'the long view' : 'your quiet record'}</p><h1 className="mt-2 font-display text-5xl tracking-tight sm:text-6xl">{title}</h1><p className="mt-3 max-w-md text-sm text-[hsl(var(--muted-foreground))]">{timelineOnly ? 'Move through the days you have kept, one thread at a time.' : 'A place to gather what happened, before it drifts away.'}</p></div><div className="flex items-center gap-2"><DateNav date={date} setDate={setDate} />{!timelineOnly && <Button onClick={() => setShowAdd(true)} data-testid="button-open-add-activity"><Plus size={16} />Add activity</Button>}</div></header><AISection date={date} /><div className="mt-12 flex items-center justify-between border-b border-[hsl(var(--border))] pb-3"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[hsl(var(--accent))]" /><span className="font-mono text-[10px] uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">{busy ? 'finding moments…' : `${events.length} ${events.length === 1 ? 'moment' : 'moments'} recorded`}</span></div>{error && <button onClick={load} className="text-xs font-semibold text-[hsl(var(--accent))] hover:underline" data-testid="button-retry-events">Try again</button>}</div>{error && <div className="mt-5 rounded-xl border border-[hsl(var(--destructive)/.25)] bg-[hsl(var(--destructive)/.06)] px-4 py-3 text-sm text-[hsl(var(--destructive))]" role="alert">{error}</div>}<div className="mt-3"><Timeline events={events} busy={busy} onDelete={remove} /></div>{showAdd && <AddActivity date={date} onClose={() => setShowAdd(false)} onCreated={load} />}</div>;
}
