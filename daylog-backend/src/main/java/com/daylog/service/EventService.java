...
public class EventService {
    ...
    public EventService(
            DayEventRepository eventRepository,
    ...
    public EventResponse create(AppUser user, EventCreateRequest request) {
        ...
        if (request.eventType() == EventType.LOCATION && place == null) {
            throw new InvalidRequestException(
                    "Location events must reference one of your saved places"
            );
        }

        if (request.eventType() == EventType.ACTIVITY
                && request.category() == null) {
            throw new InvalidRequestException(
                    "Activity events must include a category"
            );
        }

        DayEvent event = eventRepository.save(
                new DayEvent(
                        user,
                        request.eventType(),
                        request.title().trim(),
    ...
    public List<EventResponse> listForDate(
            AppUser user,
            LocalDate date
    ) {
        ...
        return eventRepository
                .findForDate(user.getId(), start, end)
                .stream()
    ...
    public void delete(
        ...
        DayEvent event = eventRepository
                .findForUser(eventId, user.getId())
                .orElseThrow(() ->
                        new ResourceNotFoundException("Event was not found")
                );
    ...
    @Transactional
    public EventResponse createDemoLocationEvent(
            AppUser user,
            DemoLocationRequest request
    ) {

        Place place = placeService.findForUser(
                user,
                request.placeId()
        );

        String action = request.action() == DemoLocationAction.ARRIVE
                ? "Arrived at "
                : "Left ";

        String defaultDescription =
                request.action() == DemoLocationAction.ARRIVE
                        ? "Arrived at " + place.getName() + "."
                        : "Left " + place.getName() + ".";
